...
ck("ledger: under the switch a top-up with a true tx identity is recorded as \"add\", one without as \"agg\"; off the switch nothing is recorded or deduplicated", count(/if \(LADDER_V2\(\)\) tr\.source\(srcId \|\| sourceId\(\{ path: "poll"/g) === 2 && count(/srcId \? "add" : "agg"\)/g) === 2 && count(/const srcId = LADDER_V2\(\) \? /g) === 2);
