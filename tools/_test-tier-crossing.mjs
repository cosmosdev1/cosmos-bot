...
ck("ladder wiring: hosted picks NC_PCTS_HOSTED, legacy NC_PCTS_LEGACY, via ONESHOT", count(/const V2_NC_PCTS = ONESHOT \? NC_PCTS_HOSTED : NC_PCTS_LEGACY, V2_CANDLE_PCTS = CANDLE_PCTS;/g) === 1 && count(/\[5, 4, 3\]/g) === 0);
ck("ceiling wiring: both posCeil sites use positionCeilPct() (hosted 6.5, legacy MAX_POSITION_PCT)", count(/\(\(state\.portfolio \|\| 0\) \* positionCeilPct\(\)\) \/ 100/g) === 2 && count(/const positionCeilPct = \(\) => \(ONESHOT \? POSITION_CEIL_PCT_HOSTED : MAX_POSITION_PCT\);/g) === 1 && count(/\* MAX_POSITION_PCT\) \/ 100/g) === 0);
ck("top-up arithmetic is the tier-crossing delta at both sites (target - held), floored at the hosted $2 minimum", count(/let add = Math\.min\(target, posCeil\) - held;/g) === 1 && count(/const add = Math\.min\(target, posCeil\) - \(Number\(mine\.size_usd\) \|\| 0\);/g) === 1 && count(/if \(add < addFloorUsd\(\)\) \{ tr\.block\("add_below_min"\)/g) === 2 && count(/const addFloorUsd = \(\) => \(ONESHOT \? V2_FLOOR_USD : MIN_ADD_USD\);/g) === 1);
...
ck("ledger: a top-up with a true tx identity is recorded as \"add\", one without as \"agg\" (never canonical)", count(/tr\.source\(srcId \|\| sourceId\(\{ path: "poll"/g) === 2 && count(/srcId \? "add" : "agg"\)/g) === 2);
...
ck("9 the only source-identity consumers are the two legacy top-up sites (no new opportunity path)", count(/tr\.source\(/g) === 2 && count(/newestFillIdOnRow\(sig\)/g) === 1 && count(/sourceId\(\{ path: "fast"/g) === 1);
